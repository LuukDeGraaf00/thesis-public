...
The third section recuperates on the initial performance considerations and benchmarks the different approaches on a raytracer.
...
Concretely it means pattern synonyms can been pattern matched against, and functions appear similar to their surface type equivalent.
...
\begin{verbatim}
    ...
    pattern Nothing_ <- (0, _)
        where Nothing_ = (0, undef)
...
In some cases it might be preferable to be nominal, which is as trivial as \type{pattern Just a = Con0}.
This will work on all sum types of \type{V [a, ()]} which one might consider to comprise type-safety in some cases.
A solution is to define a new datatype which implements all functionality through the variant type.
...
nominal implementation with pattern synonyms
