...
With this in mind it is important for high performance oriented applications to allow for flexibility, while preserving semantics.
For operating on non-uniform data there are three primary parameters which can be adjusted. 
Those are whether an element or variant-wise data structure is used, the layout for mutually exclusive variants and the ability to operate on those variants.
Within this chapter a modular interface is proposed that separates these concerns. 
...
An annoying limitation is that type families cannot be partially applied, which means a lot of boilerplate is required to capture more complex functions.
