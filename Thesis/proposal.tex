...
\section{Types (related work)}

A conclusion of the previous section can be, that the most performant internal representation cannot be deducted from a theoretical framework.
Algorithmic complexity can be used to get a general idea of the most performant solution, but not a definitive answer.
Extensive profiling and defined parameters are required, due to the many factors that can impact the performance of a program.
In a programming language, abstraction on the internal representation can be used to ergonomically change the foundation of the program.
This can be used to determine the most performant solution, due to the ease of which multiple solutions can be profiled.
Within this chapter related implementations that preserve their behavior are discussed in the context of user defined data types.


\subsection{Alignment}

\subsection{Packed/Unpacked}

Remove/add form of indirection...

\subsection{Strict/Laziness}

\subsection{AoS/SoA}

\subsection{Embedded/Framework}

\subsection{WIP}

User defined types are fundamental in many programming languages.
There exist many distinct implementations that allow for defining your own custom composite datatypes.
Within this section applicable implementations will be discussed in the context of variants, i.e. datatypes that have several representations.
...
\subsection{*Mainstream languages temp name*}
