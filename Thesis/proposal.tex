...
From a performance standpoint it might be preferable to separate the primitives on a batch-level, requiring even more datatypes.
Maintaining all of them is ergonomically not viable, which is odd when considering that all approaches are interchangeable in their intent to operate on multiple variants of a type.
...
Within this paper we propose a way to ergonomically switch between efficient variant type representations for data-parallel applications.
