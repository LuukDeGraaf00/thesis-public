...
Data-parallel array languages can operate on a higher abstraction level by distributing instructions over multiple elements. 
...
\begin{verbatim}
    nearest :: Ray -> [Triangle] -> Distance
    nearest ray = fold min 1e30f . map (intersect ray)

    float nearest(Ray ray, Triangle triangles)
    {
        float minDistance = 1e30f;
        for (int i = 0; i < objects.Length; i++)
        {
            float distance = intersect(ray, triangles[i]);
...
Sometimes it is trivial to incorporate low-level optimizations, with the example of the \type{nearest} function using the sentinel value \type{1e30f} to avoid an explicit failure state\footnote{A tag would require branching on the result, which hinders data-parallelism in the general case.}.
...
This is made apparent when attempting to extend the intersection code to operate on multiple primitives, such as spheres.
An initial consideration is whether triangles and spheres are handled on a by element-basis or if they are treated as a separate collection.

\begin{verbatim}
    data Object = Triangle .. | Sphere ...

    nearest :: Ray -> [Object] -> Distance
    nearest ray = fold min 1e30f . map (intersect ray)

    
    class Object a where
        intersect :: Ray -> a -> Distance

    nearest :: (Object a) => Ray -> [a] -> Distance
    nearest ray = fold min 1e30f . fold (map (intersect ray)) 1e30
\end{verbatim}

From a performance standpoint the collection approach appears\footnote{This is by no means absolute, as it scales poorly with the amount of primitives and a GPU would have very little overhead operating on a sorted primitive list due to non-diverging branches. } to maximize opportunities for both data-parallelism and task-parallelism.
An initial issue arises when we want to know which primitive has been hit, which can be resolved by creating a datatype that either holds a triangle or a sphere. 
...
The intersections will be performed in smaller batches which reduces the viability of separating the primitives, unless an Acceleration Structure is made for each primitive type.
Benchmarking requires both approaches to be maintained, but the dissimilarity between them make it ergonomically uncomfortable.
From a semantic perspective this is odd due to the approaches being effectively interchangeable.
The disconnect can be explained by the inability to abstract over both value-level variants and type-level variants.
...
The issue can be illustrated through raytracing, an embarrassingly parallel problem with several instances of non-uniform data.
The main instance occurs when recursively intersecting geometry.
...
In practice this is solved by either unconditionally executing subsequent rays or reordering the rays for each recursive step.
These considerations also exist outside exception states, such as handling multiple shapes of geometry efficiently in data-parallel context.  

\begin{figure}[ht]
    \centering
    \includegraphics[scale=0.3]{introduction.drawio.png}
\end{figure}

The approaches are all similar in intent, to iterate on a variant of a type, but can have significant runtime performance implications.
An ergonomic way to switch between these approaches can be essential for deducing the most performant option through benchmarking.
An efficient and type-safe abstraction that encapsulates these approaches is non-trivial, as it involves both the value and type representation.
This is apparent when attempting to define functions that can operate on both value-level variants (tagged unions) and type-level variants (types).
Unification of these concepts would reformulate operating on non-uniform data to an optimization problem.
Within this paper we propose a way to ergonomically switch between efficient representations of non-uniform data.
We preserve type-safety by elevating the concept of a mutual exclusive datatype to the type-level, which is achieved through type-level and datatype-generic programming.
An implementation can exist without it being natively supported as construct in the implementation language.
Which can be utilized by libraries, frameworks and embedded domain-specific languages that exist in languages that facilitate type-level programming and datatype-generic programming, like Haskell.
