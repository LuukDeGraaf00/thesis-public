...
While these type-indexed integrate well with type classes, type families is the much stronger standalone concept.
...
Type-level functions allow types to be computed, and as such are a way to easily construct multiple representations for a variant type.
...
The \type{Constructor} data family then constructs the actual \type{[variant] -> *} type based on the passed \type{Compact} argument.
...
While the approach is clinical, it would require all used types to be of that kind. 
...
Defining the \type{Elt} class for a specific representation of a variant is therefore made trivial.
...
Within this section a modular interface is proposed which allows for ergonomic switching between multiple representations.
...
The conversion is analogous to the \type{CustomBitSize} type family, but the the promoted list type \type{[]} is used as the monoid instead.
...
The corresponding element is removed from the comparison list once it has been matched, which preserves each individual field within the variant.
...
In this section previously discussed optimizations are benchmarked, while preserving the initial type identity of the functions to demonstrate the principle.
The benchmarks will be done with the Criterion library, and will utilize the different backends of Accelerate.
Benchmarks will be run on an {\it Intel Core i5-10600KF} CPU and a {\it NVIDIA GeForce RTX 3060 Ti} GPU.

\paragraph{Nearest}

The \type{nearest} function is used, the original motivating example, which computes the nearest primitive that intersects with a ray.
The type of primitive include spheres, planes and triangles.
A comparison is made with a variant-wise collection, which either uses multiple functions branchless (split) or a single function (joined). 
The element-wise collection are either randomly distributed or the primitives are sorted to exploit coherence.
The compact representation deduplicates all fields of equal size.
