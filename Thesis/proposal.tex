...
Within this chapter Accelerate, the implementation language, will be discussed in the context of iterating on non-uniform data.
This is extended by looking at integration of non-uniform data within other functional data-parallel array languages.   
It will be concluded by comparing existing ECS libraries and their interpretation of the underlying data structure and structural changes. 
...
Research has been done on including sum types to the Futhark compiler\cite{futhark-sum-types}, which has been implemented.
A non-compact representation is used, but references the potential to overlap only fields of the same {\it type}.  
While not the most compact, it avoids re-interpretation of undefined data. 
...
The premise is to efficiently iterate on non uniform data, which aligns with existing functionality of pure functional array languages.
This can be separated into three distinct chapters.
The first chapter investigates compatibility with existing collective array operators in Accelerate.
The second chapter attempts to identify {\it run-time} performance considerations between the discussed data structures.
This will be guiding in the third chapter, which aims to create an interface that is integrable within the type system of Haskell and provides sufficient type information for optimizations.

The research on the {\it Recursive Tagged Union} in Accelerate has centered around memory usage\cite{accelerate-sum-types}.
The Futhark paper goes in depth on type theory of structural sum types and minimal research on the performance implications\cite{futhark-sum-types}.
Within this a wide-range of data structures will be explored from a performance standpoint, which sets it apart from previous research on this subject.  
There also exist a lot of research on vectorization of non-array data structures, which employ a similar approach of directly operating on the internal data structure.
While the ECS pattern is prevalent in many applications, to my knowledge there is limited (public) research on the optimizations done within these discussed libraries.
The goal is not to implement ECS, but to use applicable optimizations from ECS within the context of sum types in functional data-parallel languages.

\begin{enumerate}
    \item Which collective array operations in Accelerate can be supported?
    \item What are the performance considerations between the data structures?
    \item What interface provides sufficient information to support optimizations on non-uniform data?
\end{enumerate}
...
As array languages operate on arrays, the functionality must be similar to remain compatible.
In principle this means {\it delegating} the collective operation to the underlying data structure, based on the type of the function.
This cannot be done when an index is explicitly used, such as the function \type{imap} as the index would be local to the underlying data structure.
Internally \type{imap} is implemented using an array of generated indices through the \type{generate} function.
In theory this means that it can be supported with minimal interference, by reimplementing these functions and replacing the way indices are provided for those functions.
This is likely less trivial for element indexing, as it requires an array of pointers/indices to be maintained.
An initial assumption is that this is unfeasible to do without sacrificing performance {\it or} it requires integration within Accelerate.
An initial implementation should provide additional context to the feasibility of this.

\newpage

\subsection{Comparison}

Preliminary benchmarks must be done on the {\it runtime} performance of the discussed data structures, for both LLVM backends.
As these are all forms of arrays and regions within arrays, existing functionality in Accelerate allows for such benchmarks. 
A non-exhaustive list of these data structures is:

\begin{enumerate}
    \item Element-wise
    \begin{enumerate}
    \item Naive tagged union
    \item Recursive tagged union
    \item Futhark suggested tagged union
    \item Entity (without using the underlying data structure)
    \end{enumerate}
    \item Variant-wise
    \begin{enumerate}
    \item An array for each variant
    \item An array for each distinct fields 
    \item Grouping within a single array
    \end{enumerate}
\end{enumerate}

Within these comparisons and benchmarks, the focus lies on identifying properties and overall performance.
For this the optimizations discussed within the background will be used.
A non-exhaustive list of these properties are:

\begin{enumerate}
    \item Vectorization (automatic)
    \item Array fusion (memory bandwidth, register pressure)
    \item Structural changes of variants (scan, sorting, grouping, allocation)
    \item Sorted arrays (cache coherence, thread divergence, branch prediction)
    \item Transform into persistent array (gather, scatter)
    \item Suitability for destructive updates and task parallelism
\end{enumerate}

The results will be guiding in deciding the feasibility of the internal representations.
This will also give an initial idea on what information is relevant for optimizations and the manual control required to create a performant program.

\subsection{Interface}

The main issue with sum types in Haskell (for this cause) is that structural changes to elements is not apparent.
There are various ways to implement structural sum types in Haskell, many of which have been implemented in numerous libraries.
A non-exhaustive list of used concepts:
...
Implementations will be based on the results of the previous section.
In Accelerate the tag and union type has not yet been fully implemented\cite{accelerate-sum-types}, which I intend to continue on.
The representation/interface for nominal types has also not yet been completed\cite{accelerate-sum-types}, which I do not intend to continue on.
...
\begin{itemize}
    \item [01-08-2023] Official Start

    \vspace{10pt}
    \item [08-08-2023] Break (1 week)
    \vspace{10pt}

    \item [15-08-2023] Preliminary implementation (1 week)
    \item [05-09-2023] Complete previous research / union (3 weeks)
    \item [12-09-2023] Preliminary Benchmarks (1 week)
    \item [26-09-2023] Draft Chapter 1 (2 weeks)
    \item [10-10-2023] Draft Chapter 2 (2 weeks)
    \item [24-10-2023] Reformulate background information (2 weeks)
    \item [31-10-2023] Complete implementation (1 week + parallel to writing)
    
    \vspace{10pt}
    \item [07-11-2023] Break (1 week)
    \vspace{10pt}

    \item [14-11-2023] Preliminary experimenting with interface (1 week)
    \item [28-11-2023] Draft Chapter 3 (2 weeks)
    \item [12-12-2023] Implementation optimizations (2 weeks)
    \item [19-12-2023] Complete interface (1 weeks + parallel to writing)
    
    \vspace{10pt}
    \item [02-01-2024] Break (2 weeks)
    \vspace{10pt}
    
    \item [30-01-2024] Write Thesis (4 weeks)
    \item [13-02-2024] Buffer (2 weeks)
\end{itemize}
