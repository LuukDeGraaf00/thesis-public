...
\begin{verbatim}
    class (Elt var, Elt rep, BitSize var <= BitSize rep) => IsVariant var rep where

        construct :: Exp var -> Exp rep
        construct = Construct

        destruct  :: Exp rep -> Exp var
        destruct = Destruct
\end{verbatim}
...
The \type{Insert} function is slightly more involved, as it is tail-recursive on the composed value.
Normally this would just be the expression that is passed along, but we are operating on the structure and do not have access to the actual expression.
