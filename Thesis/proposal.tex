...
Array languages such as APL, Accelerate\cite{accelerate} and Futhark\cite{futhark-sum-types} can operate on a higher abstraction level compared to general-purpose languages through collective operations. 
The abstraction cam be used to implicitly execute instructions in parallel on all elements within an array. 
...
This induces the need for a flexible and type-safe interface around these optimizations, which has been achieved through type-level and datatype generic programming.
...
Capturing low-level optimizations in high-level languages has been achieved by various methods; such as compiler optimizations, a Foreign Function Interface (FFI), compiler intrinsics or embedding domain-specific languages. 


For variant types in particular it involves resorting to untyped code and bitwise operations, to gain control over the memory representation and (de)-construction.
