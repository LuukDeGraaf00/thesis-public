...
\begin{itemize}
    ...
    As example, an entity can gain momentum at any time by attaching a \type{Velocity} component.
    \item [Component]
...
The ECS pattern exists on a similar abstraction-level as array languages, through the use collective operations.
To reiterate on section \ref{element-wise}, an entity is implemented as a unique identifier.
Rather than using the identifier to find the associated components, collective operations operate directly on the components.
It allows for a variant-wise collection, which most ECS implementations use to optimize the performance of individual systems.
