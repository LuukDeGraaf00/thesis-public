...
With this in mind it is important for high performance oriented applications to be flexible within the internal representation of datatypes.
...
The premise is to organizes game-logic within functions rather than components, where the relation between components is flexible\cite{ecs-origin}.
...
The pattern is often combined with data-oriented design.
...
An entity can generally only have one component.
...
Accelerate uses the \type{Elt} class to map between surface types and a internal representation.
...
It cannot be partially applied due to partially applied type synonyms requiring full fledged higher-order unification, which is not supported in Haskell.
...
One way is to use a kind more specific then \type{*} that is explicit in the composition, such as the \type{Natural} kind but for all datatypes.
...
The function \type{eltR} enforces this by requiring the associated type \type{EltR a} to have a mapping to the \type{TupR} type\footnote{Note this can circumvented by merely returning a bottom type such as \type{undefined}}. 
