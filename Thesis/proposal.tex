...
Operating on multiple variants can be done on the type-level (polymorphism) or on the value-level (tags).
In the context of iterating on a collection of variants, branching on a tag or dynamically dispatching will inherently break vectorization.
...
Such implementation can exist without it being natively supported in the implementation language.
...
A computationally efficient representation of non-uniform data in data-parallel applications.
This induced the need for a type-safe interface to such representation, which has been achieved through type-level functions and datatype-generic functions.
The connection is made through an implementation in an embedded domain-specific language, which leverages the type-system of the host language. 
...
It is not apparent what the representation of a tagged union should be, as they will inherently break vectorization. 
A majority of languages natively represent tagged unions as a singular contiguous datatype; such as C, C++, Rust and OpenCL.
Both Accelerate\cite{accelerate-sum-types} and Futhark\cite{futhark-sum-types} implicitly distribute primitive types of composite datatypes over multiple arrays, including tagged unions.
...
Many implementations incite a collective re-organization of the internal representation when a variant change occurs.
A type-safe and performant implementation is notably hard due to the statically resolving all interactions between the representations, which means meta-programming and unsafe code are extremely prevalent. 
...
\paragraph{Embedded Domain Specific Language}
