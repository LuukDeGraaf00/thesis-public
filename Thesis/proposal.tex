...
Haskell supports higher-kinded types, which are analogous to higher-order functions for types.
...
The \type{Succ a} and \type{Nil} types are axioms that can be used to construct a natural number on the type-level.
By default these exist in a open universe, which means we can create ill-formed expressions.
On the type-level this can be resolved by Generalized Algebraic Data Types (GADTs), a Haskell extension.
It allows the types of constructors to diverge from the general type, which makes the type variable a phantom type.
...
The kind \type{Natural} includes the types \type{Succ} and \type{Nil}.
It both creates a closed universe and allows type constructors to be defined with more precision than their arity.
This requires kind-level functions.
