...
There are considerations for both computational efficiency and memory efficiency, which are dependant on the positioning of the datatype within the application. 
Concretely, can the operations on the data be vectorized or do the operations require a lot of random-access.
Is the application memory-bound or compute-bound, which device is used, and which operations operate on the representation.  
...
In this chapter an interface is discussed which can capture all these different data structures in a type preserving manner without sacrificing performance. 
In the first section requirements are discussed and the way these are generally captured in other applications
In the second and third section we show how type-level programming and datatype-generic programming can be used to implement a type-level variant type.
...
An Entity-Component-System (ECS), previously mentioned in section \ref{related-work} and \ref{element-wise}, allow functions to be defined on a subset of variants.
...
In contrast to inheritance, where relations are statically determined.
...
It exists on a similar abstraction-level as array languages, by using collective operations. 
...
The canonical example of components are \type{Position} and \type{Velocity}, which when composed represent movement.
Components are generally value types to avoid race conditions.
Some implementations allow for a (readonly) reference component which shares it data along multiple instances.
A shared \type{Mesh} component would prevent excessive geometry data to be stored, which is beneficial for cache efficiency.

\paragraph{Entity}
An entity is idiomatically a set of components, but as discussed in section \ref{element-wise} an identifier of the concrete data.
...
Any immovable entity can be made movable at any time by attaching the \type{Velocity} component at runtime.

\paragraph{Systems}
A system operates on all entities that match a specific set of components.
The \type{Movement} system will operate on all movable entities, irrespective of other components attached to it.
Systems are effectively global functions, which operate only on variants that contain the required components. 
...
A performant element-wise collection would require a statically constraint scope and a notion of mutual exclusivity.
...
Deconstructing an algebraic data type is done by pattern matching on the data constructor.
The pattern match can exhaustively match on all variants, as all data constructors are known at compile-time.
...
A function that takes \type{Just a} and returns \type{Just b} would ensure that the {\it collective identity} can be preserved in a collective operation, irrespective of the concrete data transformation.
...
To remain performant, a variant type should be precise.
...
Concretely, \type{Maybe a} is now an alias for the mutually exclusive relationship between \type{Just a} or \type{Nothing}, rather than an unique and standalone type. 
...
A way to compartmentalize the difference is by considering that \type{||} is now a type constructor that takes two datatypes.
The variants are now apparent to the type system, which can be used to generically adapt data structures to the concrete variants in a variant type. 
...
The original \type{\hyperlink{fig:fmap}{fmap}} definition would force the result of the collective operation to be an element-wise collection.
