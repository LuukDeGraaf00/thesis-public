...
It cannot be partially applied due to partially applied type synonyms requiring higher-order unification, which is not supported in Haskell.
...
In the previous chapter we established a way to compute a wide-range of internal representations for a variant type.
...
This can be done through datatype-generic programming, which parametrizes to the shape of a datatype\cite{datatype-generic-programming}. 

\begin{verbatim}
    datatype generic programming
\end{verbatim}

A mapping must be isomorphic, such that all information is preserved between construction and deconstruction of the representation.
This is not possible for all possible pairings, as the variant must be smaller or equal to the representation.
